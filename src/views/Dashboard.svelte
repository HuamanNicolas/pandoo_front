<script>
  ...
  let cursosDisponibles = $derived.by(() => {
    return cursosStore.cursos.filter(curso => !cursosStore.estaInscrito(curso.id));
  });
  
  async function handleInscripcion(cursoId) {
    inscribiendoCurso = cursoId;
    
    const result = await cursosStore.inscribir(cursoId);
    
    inscribiendoCurso = null;
  }
              ...
              <div class="course-card">
                ...
                <div class="course-card-body">
                  ...
                  <div class="mb-2">
                    ...
                    <small class="text-white-50">{Math.round(curso.progreso)}% completado</small>
                  </div>
            ...
            <div class="course-card">
              <div class="course-card-body">
                ...
                <button 
                  ...
                  disabled={inscribiendoCurso === curso.id}
                >
                  <PlusCircle size={16} />
                  {inscribiendoCurso === curso.id ? 'Inscribiendo...' : 'Inscribirse'}
                </button>
              </div>
            </div>
          </div>
