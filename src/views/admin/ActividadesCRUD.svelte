<script>
  ...
  import { ClipboardList, Plus, Edit, Trash2, Search, BookOpen } from 'lucide-svelte';
  ...
  function abrirModal(actividad = null) {
    if (!cursoSeleccionado) {
      toast.warning('Selecciona un curso', {
        description: 'Primero debes seleccionar un curso para crear actividades'
      });
      return;
    }
  ...
  async function guardarActividad() {
    ...
    let result;
    if (editingActividad) {
      result = await updateActividad(cursoSeleccionado, editingActividad.id, formData);
    } else {
    ...
    if (result.success) {
      ...
      cerrarModal();
      toast.success(editingActividad ? 'Actividad actualizada' : 'Actividad creada', {
...
<div class="crud-container">
  <div class="crud-header">
    ...
    <button class="btn-add" onclick={() => abrirModal()} disabled={!cursoSeleccionado}>
      <Plus size={20} />
      Nueva Actividad
    </button>
  </div>
  ...
  {#if cursoSeleccionado}
    ...
    {:else}
              ...
              <tr>
                ...
                <td>
                  <div class="actions">
                    ...
                    <button 
                      class="btn-delete" 
                      onclick={() => eliminarActividad(actividad.id, actividad.nombre)}
                      title="Eliminar"
                    >
                      <Trash2 size={18} />
                    </button>
            ...
            {:else}
              <tr>
                <td colspan="4" class="empty-message">
                  {searchTerm ? 'No se encontraron actividades' : 'No hay actividades creadas para este curso'}
                </td>
              </tr>
            {/each}
          </tbody>
      ...
      <div class="table-footer">
        <p>Total: {actividadesFiltradas.length} actividad{actividadesFiltradas.length !== 1 ? 'es' : ''}</p>
      </div>
    {/if}
  {:else}
...
{#if showModal}
  ...
  >
    ...
    >
      ...
      <form onsubmit={(e) => { e.preventDefault(); guardarActividad(); }}>
        ...
        {#if !editingActividad}
          <div class="info-box">
            <p>💡 El ID de la actividad se generará automáticamente como <strong>actividad{actividades.length + 1}</strong></p>
          </div>
        {/if}
