<script>
  let { ejercicio, respuesta = $bindable(), mostrarResultado, esCorrecta, verificarRespuesta, siguienteEjercicio } = $props();
</script>
  ...
  <div class="opciones">
    {#each ejercicio.metadata.opciones as opcion (opcion)}
      <button
        ...
        class:correcta={mostrarResultado && opcion === ejercicio.metadata.respuesta}
        class:incorrecta={mostrarResultado && respuesta === opcion && !esCorrecta}
        onclick={() => {
          if (!mostrarResultado) {
            respuesta = opcion;
          }
