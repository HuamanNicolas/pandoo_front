          ...
          items: [
            {
              ...
              description:
                "Desbloquea todas las actividades de todos los cursos",
              quantity: 1,
...
app.get("/payment/success", (req, res) => {
  console.log("[REDIRECT] Pago exitoso", {
...
app.post("/webhook", async (req, res) => {
  try {
    if (process.env.MP_WEBHOOK_SECRET) {
      ...
      console.warn(
        "[WEBHOOK] WARNING: MP_WEBHOOK_SECRET no configurado - validación de firma deshabilitada",
      );
    }
    ...
    if (type === "payment") {
      ...
      console.log(
        `[WEBHOOK] Estado del pago ${paymentId}: ${paymentInfo.status}`,
      );
      ...
      if (paymentInfo.status === "approved") {
        ...
        if (userId !== externalReference) {
          console.error(
            `[WEBHOOK] Inconsistencia detectada: userId(${userId}) != externalReference(${externalReference})`,
          );
        ...
        if (
          ...
          currentUserData.paymentId === paymentId
        ) {
          console.log(
            `[WEBHOOK] Usuario ${userId} ya fue actualizado con paymentId ${paymentId}`,
          );
        ...
        console.log(
          `[WEBHOOK] Pago ${paymentId} no aprobado (estado: ${paymentInfo.status})`,
        );
      }
    }
